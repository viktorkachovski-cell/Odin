...
-- Saves an active list as a list template: the list's own text plus a copy of
-- every task inside it. Task runtime state is deliberately not copied, which
-- the template guard on public.tasks also enforces independently. This never
-- writes public.task_templates: a list template and a task template stay
-- separate types so a task template can be loaded on its own.
